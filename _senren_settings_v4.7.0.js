...
(function () {
  ...
  const groups = {
    "Visibility": [
      ...
      { label: "Theme Toggle", var: "--custom-dm-header-visibility", type: "switch-int", desc: "Show the theme icon in the header." },
    ...
    "Typography": [
      ...
      { label: "Bold Highlights", var: "--bold-highlight", type: "switch-int", desc: "Make highlighted text bold." },
      {
        label: "Serif", var: "--serif", type: "segment", desc: "Target word font family.", options: [
          { label: "Hiragino Mincho", val: '"Hiragino Mincho ProN", "Noto Serif JP", notoserifjp, "Yu Mincho", serif' },
      ...
      { label: "Custom Word Font", var: "--serif", type: "text", desc: "Directly type custom font name/stack to override the preset choices." },
      {
      ...
      { label: "Custom General Font", var: "--sans", type: "text", desc: "Directly type custom font name/stack to override the preset choices." },
  ...
  function getStoredThemeMode() {
    ...
    const inlineMode = normalizeThemeMode(
      document.documentElement.style.getPropertyValue(THEME_MODE_VAR)
    );
  ...
  function applyThemeMode(value) {
    ...
    localStorage.setItem("senrenTheme", mode);
    localStorage.setItem("darkMode", mode === "nord" ? "disabled" : "enabled");
    return mode;
  }
  ...
  function getThemeScopedVars() {
    ...
    return new Set(
      Object.values(groups)
        ...
        .filter(item => item && item.theme && item.var)
        .map(item => item.var)
    );
  }
  ...
  function applyPreset(settings) {
    ...
    for (const [key, value] of Object.entries(settings)) {
      if (shouldSkipPresetVar(settings, key)) continue;
  ...
  function buildMenu() {
    ...
    modal.querySelectorAll('input, textarea').forEach(input => {
      ...
      const handler = () => {
        ...
        // Sync segment buttons if a custom text field is updated
        const segmentGroup = document.querySelector(`.senren-segment-group[data-var="${varName}"]`);
        if (segmentGroup) {
          const clean = (s) => s ? s.toString().toLowerCase().replace(/['"\s]/g, '') : '';
          const cleanVal = clean(newVal);
          const buttons = segmentGroup.querySelectorAll('.senren-segment-btn');
          buttons.forEach((btn, idx) => {
            const btnVal = btn.getAttribute('data-val');
            if (clean(btnVal) === cleanVal) {
              btn.classList.add('active');
            } else {
              btn.classList.remove('active');
            }
  ...
  window.updateSegment = function (btn, varName, val) {
    ...
    // Sync any text input fields targeting the same variable
    const syncInput = document.querySelector(`.senren-input[data-var="${varName}"]`);
    if (syncInput) {
      syncInput.value = val;
    }
